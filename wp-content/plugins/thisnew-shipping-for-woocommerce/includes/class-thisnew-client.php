 ...
 * thisnew API client
 */
class ThisNew_Client {

	private $key = false;
	private $lastResponseRaw;
	private $lastResponse;
	private $userAgent = 'ThisNew WooCommerce Plugin';
	private $apiUrl;

	/**
	 * @param string $key thisnew Store API key
	 * @param bool|string $disable_ssl Force HTTP instead of HTTPS for API requests
	 *
	 * @throws thisnewException if the library failed to initialize
	 */
	public function __construct( $key = '', $disable_ssl = false ) {

		$key = (string) $key;

		$this->userAgent .= ' ' . ThisNew_Base::VERSION . ' (WP ' . get_bloginfo( 'version' ) . ' + WC ' . WC()->version . ')';

		if ( ! function_exists( 'json_decode' ) || ! function_exists( 'json_encode' ) ) {
			throw new ThisNewException( 'PHP JSON extension is required for the thisnew API library to work!' );
		}
		if ( strlen( $key ) < 36 ) {
			throw new ThisNewException( 'Missing or invalid thisnew store key!' );
		}
		$this->key = $key;
		...
		//setup api host
		$this->apiUrl = ThisNew_Base::get_thisnew_api_host();
	}
     ...
     * @throws thisnewApiException if the API call status code is not in the 2xx range
     * @throws thisnewException if the API call has failed or the response is invalid
     */
	...
	private function request( $method, $path, array $params = array(), $data = null ) {
		...
		$result = apply_filters( 'thisnew_api_result', $result, $method, $this->apiUrl . $url, $request );

		if ( is_wp_error( $result ) ) {
			throw new ThisNewException( "API request failed - " . $result->get_error_message() );
		}
		$this->lastResponseRaw = $result['body'];

		$this->lastResponse    = $response = json_decode( $result['body'], true );

        if ( ! isset( $result['response']['code'], $response['data'] ) ) {
			throw new ThisNewException( 'Invalid API response' );
		}
		$status = (int) $result['response']['code'];
		if ( $status < 200 || $status >= 300 ) {
			throw new ThisNewApiException( (string) $response['data'], $status );
		}
...
/**
 * Class ThisNewException Generic ThisNew exception
 */
class ThisNewException extends Exception {}
/**
 * Class ThisNewException ThisNew exception returned from the API
 */
class ThisNewApiException extends ThisNewException {}