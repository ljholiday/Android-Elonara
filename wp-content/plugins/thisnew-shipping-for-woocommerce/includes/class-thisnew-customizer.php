...
class ThisNew_Customizer {
    ...
    public function hook_actions()
    {
        ...
        add_action( 'wp_ajax_thisnew_customized_thumb', array( $this, 'thisnew_customized_thumb' ) );
        add_action( 'wp_ajax_nopriv_thisnew_customized_thumb', array( $this, 'thisnew_customized_thumb' ) );
    ...
    public function load_customizer_scripts() {
        ...
        if ($is_customized_product || is_cart()) {
        	$customizer_script_handle = 'thisnew-product-customizer';
            ...
            wp_localize_script( $customizer_script_handle, 'pfGlobal', array(
                ...
                'modal_title' => ThisNew_Integration::instance()
                    ->get_option( 'pfc_modal_title', ThisNew_Admin_Settings::DEFAULT_PERSONALIZE_MODAL_TITLE )
            ));
        }
    }
    ...
    public function change_woocommerce_cart_item_thumbnail( $thumbnail, $cart_item )
    {
        if ( $cart_item
            ...
            $src = esc_url(ThisNew_Base::get_asset_url() . 'images/loading.gif' );
            // Construct img tag.
	 ...
	 * @throws Exception
	 */
    public static function thisnew_customized_thumb()
    {
