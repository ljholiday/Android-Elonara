...
class ThisNew_Admin_Status {
	...
	const PF_WEBHOOK_NAME = 'ThisNew Integration';
	const PF_REMOTE_REQUEST_URL = 'hook/woocommerce?store=1';
	const PF_REMOTE_REQUEST_TOPIC = 'woo.plugin.test';
	const PF_STATUS_ISSUE_COUNT = 'thisnew_status_issue_count';
	const PF_CACHED_CHECKLIST = 'thisnew_cached_checklist';
	...
	public static function getChecklistItems() {
	    return array(
            array(
                'name'        => __( 'Connection to ThisNew API', 'thisnew' ),
                'description' => __( 'Is your store successfully connected to ThisNew API?', 'thisnew' ),
                'method'      => 'check_PF_API_connect',
            ),
            array(
                'name'        => __( 'ThisNew API key is set', 'thisnew' ),
                'description' => __( 'Your store needs access to ThisNew API to use most of it\'s features like shipping rates, tax rates and other settings.', 'thisnew' ),
                'method'      => 'check_PF_API_key',
            ),
            array(
                'name'        => __( 'WordPress Permalinks', 'thisnew' ),
                'description' => __( 'WooCommerce API will not work unless your permalinks in Settings > Permalinks are set up correctly. Make sure you that they are NOT set to "plain".', 'thisnew' ),
                'method'      => 'check_permalinks',
            ),
            array(
                'name'        => __( 'WordPress version', 'thisnew' ),
                'description' => __( 'WordPress should always be updated to the latest version. Updates can be installed from your WordPress admin dashboard.', 'thisnew' ),
                'method'      => 'check_WP_version',
            ),
            array(
                'name'        => __( 'WooCommerce Webhooks', 'thisnew' ),
                'description' => __( 'ThisNew requires WooCommerce webhooks to be set up to quickly capture you incoming orders, products updates etc.', 'thisnew' ),
                'method'      => 'check_PF_webhooks',
            ),
            array(
                'name'        => __( 'WooCommerce API keys are set', 'thisnew' ),
                'description' => __( 'ThisNew needs access to your WooCommerce API for the integration to work - otherwise we can\'t sync your store, push or pull your products etc.', 'thisnew' ),
                'method'      => 'check_WC_API_access',
            ),
            array(
                'name'        => __( 'WooCommerce authentication URL access', 'thisnew' ),
                'description' => __( 'ThisNew needs access to WooCommerce API authorize page. This sometimes may get blocked due to hosts having unnecessarily intrusive security checks in place that prevent WooCommerce API authentication from working (for example mod_security rule #1234234). If this check fails, you will not be able authorize ThisNew app.', 'thisnew' ),
                'method'      => 'check_WC_auth_url_access',
            ),
            array(
                'name'        => __( 'WordPress remote requests', 'thisnew' ),
                'description' => __( 'WordPress needs to be able to connect to ThisNew server to call webhooks. If this check fails, contact your hosting support.', 'thisnew' ),
                'method'      => 'check_remote_requests',
            ),
            array(
                'name'        => __( 'WordPress Site URL', 'thisnew' ),
                'description' => __( 'If your currently setup WordPress site URL is redirected to another URL the integration might not work correctly. Typically this happens with incorrect http to https redirects. Go to Settings > General to fix this.' , 'thisnew' ),
                'method'      => 'check_site_url_redirect',
            ),
            array(
                'name'        => __( 'Recent store sync errors', 'thisnew' ),
                'description' => __( 'ThisNew will connect to your store\'s API regularly and sync your latest products, orders etc. If there have been any recent issues with sync, this check will fail.', 'thisnew' ),
                'method'      => 'check_PF_sync_errors',
            ),
            array(
                'name'        => __( 'Write permissions', 'thisnew' ),
                'description' => __( 'Make the uploads directory writable. This is required for mockup generator product push to work correctly. Contact your hosting provider if you need help with this.', 'thisnew' ),
                'method'      => 'check_uploads_write',
            ),
            array(
                'name'        => __( 'PHP memory limit', 'thisnew' ),
                'description' => __( 'Set PHP allocated memory limit to at least 128mb. Contact your hosting provider if you need help with this.', 'thisnew' ),
                'method'      => 'check_PHP_memory_limit',
            ),
            array(
                'name'        => __( 'PHP script time limit', 'thisnew' ),
                'description' => __( 'Set PHP script execution time limit to at least 30 seconds. This is required to successfully push products with many variants. Contact your hosting provider if you need help with this.', 'ThisNew' ),
                'method'      => 'check_PHP_time_limit',
            ),
            array(
                'name'        => __( 'W3 Total Cache DB Cache', 'thisnew' ),
                'description' => __( 'If you are using W3 Total Cache, the database caching feature needs to be disabled since it can cause issues with product push to store.', 'thisnew' ),
                'method'      => 'check_W3_db_cache',
                'silent'      => true,
            ),
            array(
                'name'        => __( 'WP SpamShield', 'thisnew' ),
                'description' => __( 'If you are using WP SpamShield, you might experience problems connecting to ThisNew and pushing products.', 'thisnew' ),
                'method'      => 'check_wp_spamshield',
                'silent'      => true,
            ),
            array(
                'name'        => __( 'Remove Print Aura plugin', 'thisnew' ),
                'description' => __( 'Print Aura plugin is known to cause issues so it needs to be removed.', 'thisnew' ),
	...
	public function render() {
		...
		if ( $checklist ) {
            ThisNew_Admin::load_template( 'status-table', array( 'checklist' => $checklist ) );
		} else {
            ThisNew_Admin::load_template( 'ajax-loader', array( 'action' => 'get_thisnew_status_checklist', 'message' => __( 'Testing your store (this may take up to 30 seconds)...', 'thisnew' ) ) );
		}
	...
	public static function get_checklist( $only_cached_results = false ) {
		...
		$list = get_transient( ThisNew_Admin_Status::PF_CACHED_CHECKLIST );
		...
		foreach ( self::getChecklistItems() as $item ) {
			...
			if ( method_exists( $status, $item['method'] ) ) {
				...
				// Do not display status for methods that are depending on connection status to ThisNew
				if ( ! ThisNew_Integration::instance()->is_connected(true) && in_array( $item['method'], self::STATUS_CONNECTED_DISPLAYABLE_METHODS ) ) {
				    continue;
                }
		...
		set_transient( ThisNew_Admin_Status::PF_CACHED_CHECKLIST, $list, MINUTE_IN_SECONDS );
		set_transient( ThisNew_Admin_Status::PF_STATUS_ISSUE_COUNT, $issueCount, HOUR_IN_SECONDS );
	...
	private function check_WC_API_access() {
		...
		$queryArg = '%'. $wpdb->esc_like( $thisnewKey ) . '%';
	...
	private function check_PF_API_key() {
		...
		$option = get_option( 'woocommerce_thisnew_settings', array() );
		if ( ! empty( $option['thisnew_key'] ) && strlen( $option['thisnew_key'] ) == 36 ) {
			return self::PF_STATUS_OK;
		}
	...
	private function check_PF_sync_errors() {
		...
		$sync_log = get_option( ThisNew_Request_log::PF_OPTION_INCOMING_API_REQUEST_LOG, array() );
		if ( empty( $sync_log ) ) {
	...
	private function check_remote_requests() {
		...
		// Webhook away!
		$response = wp_safe_remote_request( ThisNew_Base::get_thisnew_api_host() . self::PF_REMOTE_REQUEST_URL, $http_args );
	...
	private function check_WC_auth_url_access() {
		$url       = home_url( '/' ) . 'wc-auth/v1/authorize?app_name=ThisNew&scope=read_write&user_id=1&return_url=https%3A%2F%2Fwww.thisnew.com%2Fdashboard%2Fwoocommerce%2Freturn&callback_url=https%3A%2F%2Fapi.thisnew.com%2Fhook%2Fwoocommerce-auth-callback';
		$http_args = array(
			'timeout'    => 60,
			'method'     => 'GET',
