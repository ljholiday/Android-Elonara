...
class ThisNew_Shipping extends WC_Shipping_Method
{
    ...
    const THISNEW_SHIPPING = 'thisnew_shipping';

    //Store whether currently processed package contains thisnew products (for WC<2.6)
    private $thisnew_package = true;
	...
	public function __construct() {
		...
		$this->id                 = 'thisnew_shipping';
		$this->method_title       = $this->title = 'ThisNew Shipping';
		$this->method_description = 'Calculate live shipping rates based on actual ThisNew shipping costs.';
		...
		//Remove other shipping methods for thisnew package on WC < 2.6
	...
	/**
	 * Init fields for thisnew Shipping form
	 */
	public function init_form_fields() {
		...
		$this->form_fields = array(
			...
			'override_defaults' => array(
				...
				'type'    => 'checkbox',
				'label'   => __( 'Disable standard Woocommerce rates for products fulfilled by ThisNew', 'woocommerce' ),
				'default' => 'yes',
			),
			'show_warnings'     => array(
				'title'   => __( 'Show ThisNew warnings', 'woocommerce' ),
				'type'    => 'checkbox',
				'label'   => __( 'Display ThisNew status messages if rate API request fails', 'woocommerce' ),
				'default' => 'yes',
			),
		);

		if ( !ThisNew_Integration::instance()->get_option('thisnew_key') ) {
			$this->form_fields['info'] = array(
				'type'        => 'title',
				'description' => 'Please add ThisNew API key to the
                    <a href="' . admin_url( 'admin.php?page=thisnew-dashboard&tab=settings' ) . '">ThisNew Integration settings section</a>
	...
	/**
	 * Enable only thisnew shipping method for thisnew packages
	 * @param array $package
	 */
	public function woocommerce_load_shipping_methods( $package = array() ) {

		$this->thisnew_package = false;

		if ( $package && ! empty( $package['thisnew'] ) ) {
			if ( $this->enabled == 'yes' ) {
				$this->thisnew_package = true;
				if ( $this->override_defaults ) {
					//Remove default methods if we process thisnew package
					WC()->shipping()->unregister_shipping_methods();
				}
				WC()->shipping()->register_shipping_method( $this );
			}
		} else if ( ! $package ) {
			//Show thisnew tab on Shipping rate settings
			WC()->shipping()->register_shipping_method( $this );
		}
	}

	/**
	 * Remove non-thisnew methods for thisnew packages on WC < 2.6
	 * @param $methods
	 *
	 * @return array
	 */
	public function woocommerce_shipping_methods( $methods ) {
		...
		if ( $this->override_defaults && $this->thisnew_package && version_compare( WC()->version, '2.6', '<' ) ) {
	...
	public function woocommerce_cart_shipping_packages( $packages = array() ) {
		//thisnew rates are turned off, do not split products
		if ( $this->enabled !== 'yes' ) {
			return $packages;
		}
		...
		foreach ( $packages as $package ) {
			...
			if ( $ids ) {
				...
				$key          = 'thisnew_productids_' . md5( $values );
				$thisnew_ids = get_transient( $key );
				if ( ! is_array( $thisnew_ids ) ) {
					$thisnew_ids = array();
					try {
						...
						if ( ! empty( $status['sync_variants'] ) ) {
							foreach ( $status['sync_variants'] as $variant ) {
								if ( $variant['synced'] ) {
									$thisnew_ids[] = $variant['external_id'];
								}
							}
						}
						set_transient( $key, $thisnew_ids, 1800 );
					} catch ( ThisNewException $e ) {
						$this->set_error( $e );

						//Failed to get thisnew status, return default packages
						return $packages;
					}
				}
			}
			...
			foreach ( $ids as $key => $external_id ) {
				...
				if ( in_array( $external_id, $thisnew_ids ) ) {
					$new_contents['thisnew'][ $key ] = $item;
			...
			if ( $new_contents['virtual'] ) {
				if ( $new_contents['thisnew'] && ! $new_contents['woocommerce'] ) {
					$new_contents['thisnew'] += $new_contents['virtual'];
				} else {
	...
	public function calculate_shipping( $package = array() ) {
		...
		try {
			$key      = 'thisnew_rates_' . md5( json_encode( $request ) );
	...
	public function show_error( $data ) {
		...
		if ( $error instanceof ThisNewApiException && $error->getCode() == 401 ) {
