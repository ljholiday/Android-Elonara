...
class ThisNew_Request_log {

	const PF_USER_AGENT = 'ThisNew WooCommerce Integration';
	const PF_OPTION_LAST_API_RESPONSE = 'thisnew_last_api_response';
	const PF_OPTION_INCOMING_API_REQUEST_LOG = 'thisnew_incoming_api_request_log';
	const PF_OPTION_OUTGOING_API_REQUEST_LOG = 'thisnew_outgoing_api_request_log';
	const PF_INCOMING_API_ERRORS = 'thisnew-incoming-api-errors';
	const PF_OUTGOING_API_ERRORS = 'thisnew-outgoing-api-errors';

	public static function init() {
		$thisnew_log = new self;
		add_filter( 'woocommerce_api_serve_request', array( $thisnew_log, 'log_incoming_thisnew_api_requests' ), 10, 3 );
		add_filter( 'thisnew_api_result', array( $thisnew_log, 'log_outgoing_thisnew_api_requests'), 10, 4 );
	}

	/**
	 * Log thisnew API errors and save the last 20 API responses
	 * @param $served
	 * @param $result
	 * @param $request
	 *
	 * @return mixed
	 */
	public function log_incoming_thisnew_api_requests( $served, $result, $request ) {

		if ( ! $this->isThisNewApiRequest( $request ) ) {
			return $served;
		}
		...
		$this->save_to_thisnew_log( $request->method . ' ' . $request->path, $result, $response_hash, self::PF_OPTION_INCOMING_API_REQUEST_LOG );
	 ...
	 */
	public function log_outgoing_thisnew_api_requests($result, $method, $url, $request) {
		...
		//save summary in database to be easily accessible for status page
		$this->save_to_thisnew_log( $method . ' ' . $url, $result, $response_hash,  self::PF_OPTION_OUTGOING_API_REQUEST_LOG );

		return $original_result; //don't change the result
	}

	/**
	 * Write thisnew API request errors to log
	 *
	 * @param $request
	 * @param $result
	 ...
	 * @param $log
	 */
	private function save_to_thisnew_log( $request_title, $result, $response_hash, $log ) {
	...
	/**
	 * Check requests header for indications that this is a thisnew api request
	 * @param $request
	 * @return bool
	 */
	private function isThisNewApiRequest( $request ) {
