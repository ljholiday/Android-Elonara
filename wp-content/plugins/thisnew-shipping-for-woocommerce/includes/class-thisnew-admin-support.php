...
class ThisNew_Admin_Support {

    public static $_instance;

    /**
     * @return thisnew_Admin_Support
     */
...
//		ThisNew_Admin::load_template( 'ajax-loader', array( 'action' => 'get_thisnew_status_report', 'message' => __( 'Building support report (this may take up to 30 seconds)...', 'thisnew' ) ) );

		ThisNew_Admin::load_template( 'support-info' );
	...
	public static function render_status_report_ajax() {
		...
		ThisNew_Admin::load_template( 'status-report', array( 'status_report' => $status_report ) );
	...
	public function generate_report() {
		...
		echo __( "##### thisnew Checklist #####\n", 'thisnew' );
		$checklist = ThisNew_Admin_Status::get_checklist();
		foreach ( $checklist['items'] as $item ) {
			$status = 'OK';
			if($item['status'] == ThisNew_Admin_Status::PF_STATUS_WARNING) {
				$status = 'WARNING';
			} else if($item['status'] == ThisNew_Admin_Status::PF_STATUS_FAIL) {
				$status = 'FAIL';
			} else if ($item['status'] == ThisNew_Admin_Status::PF_STATUS_NOT_CONNECTED) {
	...
	/**
	 * Returns log of last incoming API requests from thisnew
	 * @return array
	 */
	public function get_sync_report() {

		$report      = array();
		$request_log = get_option( ThisNew_Request_log::PF_OPTION_INCOMING_API_REQUEST_LOG, array() );
