...
class ThisNew_Taxes {
	...
	public static function init() {
		if ( ThisNew_Integration::instance()->get_option( 'calculate_tax' ) == 'yes' ) {
	...
	public function calculate_tax( $matched_tax_rates, $country, $state, $postcode, $city, $tax_class ) {
		...
		if ( isset( $countries[ $country ][ $state ] ) && !empty($postcode) ) {     //only make the request if country, state and zip are set
			$key  = 'thisnew_tax_rate_' . $country . '-' . $state . '-' . $city . '-' . $postcode;
			...
			if ( $rate['required'] ) {
				...
				if ( $this->isRateUnique( $rate_item, $matched_tax_rates ) ) {
					$id                       = $this->get_thisnew_rate_id( $country, $state, $rate['shipping_taxable'] );
	...
	/**
	 * Gets list of countries and states where thisnew needs to calculate sales tax
	 * @return array|mixed
	 */
	private function get_tax_countries() {
		$countries = get_transient( 'thisnew_tax_countries' );
		...
		if ( ! $countries ) {
			...
			try {
				...
				if ( ! empty( $countries ) ) {
					set_transient( 'thisnew_tax_countries', $countries, 6 * 3600 );
				}
			} catch ( Exception $e ) {
	...
	/**
	 * Creates dummy tax rate ID to display thisnew tax rates in the cart summary.
	 *
	 * @param $cc
	 * @param $state
	 ...
	 */
	private function get_thisnew_rate_id( $cc, $state, $includeShipping = false ) {
		global $wpdb;

		$includeShipping = (int) $includeShipping;

		$states    = WC()->countries->get_states( $cc );
		$tax_title = ( isset( $states[ $state ] ) ? $states[ $state ] . ' ' : '' ) . __( 'Sales Tax', 'thisnew' );
		$id        = $wpdb->get_var(
			$wpdb->prepare( "SELECT tax_rate_id FROM {$wpdb->prefix}woocommerce_tax_rates WHERE tax_rate_class='thisnew'
